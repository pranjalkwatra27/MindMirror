...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    const loadUser = useCallback(async () => {
        try {
            const savedToken = Cookies.get('mindmirror_token') || Cookies.get('evolveai_token');
            if (savedToken) {
                setToken(savedToken);
            ...
            Cookies.remove('mindmirror_token');
    ...
    const login = async (email: string, password: string) => {
        ...
        Cookies.set('mindmirror_token', data.token, { expires: 7 });
        setToken(data.token);
        setUser(data.user);
    };
